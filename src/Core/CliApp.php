    ...
    public function run_comparison($skipFilters = false) {
        ...
        try {
            ...
            if($skipFilters != "nofilter"){
                ...
                $this->objCompare->addFilter(Utility::cutoutFilterFactory("<textarea name='debugInfo'", '</textarea>', $cliTerm));
                $this->objCompare->addFilter(Utility::cutoutFilterFactory("<div id=\"sfwdt", "</script>", $cliTerm));

//                $regexps = $this->configs['normalise']['regexp'];
//                $this->objCompare->addFilter(function($c) use ($cliTerm, $regexps){
//                        foreach($regexps as $regexp){
//                            $c = preg_replace($regexp[0], $regexp[1], $c);
//                        }
//                        return $c;
//                    });
            }else{
            ...
            $this->objCompare->fetch($notifyCallback);
